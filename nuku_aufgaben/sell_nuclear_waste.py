...
def travel_position_and_sell_nuclear_waste(x, y):
    """
    Travel to position and sell all nuclear waste (NUCLEAR_WASTE)
    """
    nuclear_waste_amount = get_resource_amount("NUCLEAR_WASTE")
    
    if nuclear_waste_amount <= 0:
        print("ℹ️  No NUCLEAR_WASTE to sell!")
        return
    
    print(f"💰 Selling {nuclear_waste_amount} units of NUCLEAR_WASTE")
    ...
    comm.sell(station_name, "NUCLEAR_WASTE", nuclear_waste_amount)
    
    print(f"✅ Sold {nuclear_waste_amount} NUCLEAR_WASTE at {station_name}")
...
def sell_all_nuclear_waste_at_nuku():
    ...
    """
    print("🚀 Selling all NUCLEAR_WASTE at Nuku Station")
    ...
    # Show current amount
    current_amount = get_resource_amount("NUCLEAR_WASTE")
    print(f"📦 Current NUCLEAR_WASTE: {current_amount}")
    
    if current_amount > 0:
        travel_position_and_sell_nuclear_waste(nuku_x, nuku_y)
        
        # Show updated amount
        remaining = get_resource_amount("NUCLEAR_WASTE")
        print(f"📦 Remaining NUCLEAR_WASTE: {remaining}")
        print("🎉 Mission complete!")
    else:
        print("❌ No NUCLEAR_WASTE to sell!")
